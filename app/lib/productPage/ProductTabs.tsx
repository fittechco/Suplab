...
function TabHeading(props: {
  ...
  return (
    ...
    >
      <h3 className={`product-tabs-title capitalize font-bold text-2xl`}>
        {title}
      </h3>
    </button>
  );
...
export default function ProductTabs(props: Props) {
  ...
  const isMetafieldsEmpty =
    metafields.find((metafield) => metafield != null) == null;
    ...
    <div className="product-tabs md:container space-y-5">
      <div className="mobile-accordions flex flex-col gap-5 md:hidden">
        ...
        {metafields.map((metafield, index) => {
          ...
          let image: Pick<ImageType, 'url' | 'width' | 'altText' | 'height'> =
            null as any as ImageType;
      ...
      <div className="product-tabs-titles flex justify-center gap-5 max-md:hidden">
        {product?.description != '' && (
          <TabHeading
            title="description"
            isSelected={selectedTabIndex === metafields.length}
            setSelectedTabIndex={() => {
